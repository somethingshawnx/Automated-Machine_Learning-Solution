...
def show_train_page(df: pd.DataFrame):
    ...
    if target_variable not in df.columns:
        # This can happen if the target was e.g. one-hot encoded by mistake
        st.error(f"Target variable '{st.session_state['target_variable']}' not found in processed data columns.")
        st.info("This might happen if the target variable was encoded. Please check your 'Preprocess' settings.")
        return
